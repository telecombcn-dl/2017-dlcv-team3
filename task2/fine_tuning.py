...
#classify using terrasa buildings
#build top_model
...
checkpoint = ModelCheckpoint("vgg16_2.h5", monitor='val_acc', verbose=1, save_best_only=True,
                             save_weights_only=False, mode='auto', period=1)
early = EarlyStopping(monitor='val_acc', min_delta=0, patience=10, verbose=1, mode='auto')

# fine-tune the model
#hist = model_final.fit_generator(train_generator, samples_per_epoch = nb_train_samples, epochs = epochs, validation_data=validation_generator,
#                                 nb_val_samples = nb_validation_samples, callbacks=[checkpoint, early])

arch = model_final.fit(X_train, Y_train, batch_size=batch_size, callbacks=[checkpoint, early], epochs=epochs, validation_data=[X_test,Y_test])

with open ('imagenet_tarrassa.txt','w') as f:
